...
FACTOR_PARTITIONS = {
    'style':
    ['momentum', 'size', 'value', 'reversal_short_term', 'volatility'],
    'sector': [
...
def create_full_tear_sheet(returns,
    ...
    positions = utils.check_intraday(estimate_intraday, returns, positions,
                                     transactions)
    ...
    if positions is not None:
        create_position_tear_sheet(returns,
                                   positions,
        ...
        if transactions is not None:
            create_txn_tear_sheet(returns,
                                  ...
                                  set_context=set_context)
            if round_trips:
                create_round_trip_tear_sheet(returns=returns,
                                             positions=positions,
                                             transactions=transactions,
                                             sector_mappings=sector_mappings,
                                             estimate_intraday=False)
            ...
            if market_data is not None:
                create_capacity_tear_sheet(returns,
                                           positions,
                                           transactions,
                                           market_data,
        ...
        if factor_returns is not None and factor_loadings is not None:
            create_perf_attrib_tear_sheet(returns,
                                          positions,
                                          factor_returns,
                                          factor_loadings,
                                          transactions,
...
def create_simple_tear_sheet(returns,
    ...
    if positions is not None:
        ...
        plotting.plot_long_short_holdings(returns,
                                          positions_alloc,
...
def create_returns_tear_sheet(returns,
    ...
    plotting.plot_rolling_returns(returns,
                                  ...
                                  live_start_date=live_start_date,
                                  cone_std=None,
                                  volatility_match=(benchmark_rets
                                                    is not None),
                                  legend_loc=None,
    ...
    plotting.plot_rolling_volatility(returns,
                                     factor_returns=benchmark_rets,
                                     ax=ax_rolling_volatility)
    ...
    plotting.plot_return_quantiles(returns,
                                   live_start_date=live_start_date,
...
@plotting.customize
def create_position_tear_sheet(returns,
                               positions,
    ...
    if sector_mappings is not None:
        ...
        if len(sector_exposures.columns) > 1:
            ...
            plotting.plot_sector_allocations(returns,
                                             sector_alloc,
                                             ax=ax_sector_alloc)
...
@plotting.customize
def create_txn_tear_sheet(returns,
                          positions,
                          transactions,
                          unadjusted_returns=None,
                          estimate_intraday='infer',
...
@plotting.customize
def create_round_trip_tear_sheet(returns,
                                 positions,
                                 transactions,
                                 sector_mappings=None,
                                 estimate_intraday='infer',
    ...
    sns.displot(trades.returns.dropna() * 100,
                kde=False,
...
@plotting.customize
def create_interesting_times_tear_sheet(returns,
                                        benchmark_rets=None,
                                        legend_loc='best',
                                        return_fig=False):
    """
    ...
    if not rets_interesting:
        warnings.warn(
            'Passed returns do not overlap with any'
            'interesting times.', UserWarning)
        return

    utils.print_table(pd.DataFrame(rets_interesting).describe().transpose().
    ...
    for i, (name, rets_period) in enumerate(rets_interesting.items()):
        ...
        ep.cum_returns(rets_period).plot(ax=ax,
                                         color='forestgreen',
                                         label='algo',
                                         alpha=0.7,
                                         lw=2)
        ...
        if benchmark_rets is not None:
            ep.cum_returns(bmark_interesting[name]).plot(ax=ax,
                                                         color='gray',
                                                         label='benchmark',
                                                         alpha=0.6)
            ax.legend(['Algo', 'benchmark'],
                      loc=legend_loc,
                      frameon=True,
                      framealpha=0.5)
        else:
...
@plotting.customize
def create_capacity_tear_sheet(returns,
                               positions,
                               transactions,
                               market_data,
                               liquidation_daily_vol_limit=0.2,
    ...
    max_days_by_ticker.index = (max_days_by_ticker.index.map(
        utils.format_asset))

    print("Whole backtest:")
    utils.print_table(max_days_by_ticker[
    ...
    llt = capacity.get_low_liquidity_transactions(transactions,
                                                  market_data,
                                                  last_n_days=last_n_days)
    ...
    plotting.plot_capacity_sweep(returns,
                                 transactions,
                                 market_data,
                                 bt_starting_capital,
                                 min_pv=100000,
...
def create_bayesian_tear_sheet(returns,
    ...
    trace_t, ppc_t = bayesian.run_model('t',
                                        df_train,
    ...
    if stoch_vol:
        ...
        previous_time = timer("running stochastic volatility model",
                              previous_time)

        # plot latent volatility
        row += 1
        ax_volatility = plt.subplot(gs[row, :])
        bayesian.plot_stoch_vol(df_train_truncated,
                                trace=trace_stoch_vol,
                                ax=ax_volatility)
        previous_time = timer("plotting stochastic volatility model",
                              previous_time)
...
def create_risk_tear_sheet(positions,
    ...
    if style_factor_panel is not None:
        ...
        for item in style_factor_panel.items:
            new_style_dict.update(
                {item: style_factor_panel.loc[item].loc[idx]})
        style_factor_panel = pd.Panel()
    ...
    if sectors is not None:
        ...
        risk.plot_sector_exposures_longshort(long_exposures,
                                             short_exposures,
    ...
    if factor_partitions is not None:
        ...
        for factor_type, partitions in factor_partitions.iteritems():
            ...
            perf_attrib.plot_risk_exposures(
                portfolio_exposures[portfolio_exposures.columns.intersection(
                    partitions)],
                ax=plt.subplot(gs[current_section]),
